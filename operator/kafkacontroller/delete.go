...
func (p *pipeline) Delete(ctx context.Context, mg resource.Managed) (managed.ExternalDelete, error) {
	...
	if !ok {
		return managed.ExternalDelete{}, fmt.Errorf("invalid managed resource type %T for kafka connection", mg)
	}
	...
	if err != nil {
		...
		return managed.ExternalDelete{}, fmt.Errorf("cannot delete kafka instance: %w", err)
	}
