...
func (p *IAMKeyPipeline) createIAMKey(ctx *pipelineContext) error {
	...
	// send request
	resp, err := ExecuteRequest(ctx, "GET", ctx.iamKey.Spec.ForProvider.Zone, "/v2/iam-role", p.apiKey, p.apiSecret, nil)
	if err != nil {
		return err
	}
	...
	if !iamRoleNameFound {
					...
					AdditionalProperties: map[string]exooapi.IamServicePolicy{
						"sos": {
							...
							Rules: &[]exooapi.IamServicePolicyRule{
								{
									Action:     &policyDeny,
									Expression: pointer.String("resources.bucket != " + strings.Split(iamKey.Spec.ForProvider.KeyName, ".")[1]),
								},
								{
									Action:     &policyDeny,
									Expression: pointer.String("false"),
								},
							},
						},
					},
		...
		fmt.Println("moje ID: ", iamRole.Id)
	...
	// send request
	resp, err = ExecuteRequest(ctx, "GET", ctx.iamKey.Spec.ForProvider.Zone, "/v2/api-key", p.apiKey, p.apiSecret, nil)
	if err != nil {
	...
	if !iamKeyFound {
		...
		_, err = ExecuteRequest(ctx, "GET", ctx.iamKey.Spec.ForProvider.Zone, "/v2/iam-role/"+iamRoleID, p.apiKey, p.apiSecret, nil)
		if err != nil {
			log.Error(err, "IAM Role doesn't exist (yet), sleeping 3 seconds before creating a key", "iamrole", iamRoleID)
		...
		fmt.Println(iamRoleID, "MOJE IAMROLEID", iamRoleID)
